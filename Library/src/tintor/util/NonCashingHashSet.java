...
@SuppressWarnings("unchecked") public final class NonCashingHashSet<T> extends AbstractSet<T> {
	Entry<T>[] table;
	int size;
	private int threshold;
	private final float loadFactor;

	public NonCashingHashSet() {
		this(16);
	}

	public NonCashingHashSet(final int initialCapacity) {
		this(initialCapacity, 0.75f);
	}

	public NonCashingHashSet(final int initialCapacity, final float loadFactor) {
	...
	static int index(final int h, final int length) {
		return h & length - 1;
	}
	...
	@Override public boolean add(final T key) {
		final int i = index(key.hashCode(), table.length);
		...
		for (int j = 0; j < table.length; j++) {
			...
			if (e != null) {
				...
				do {
					...
					final int i = index(e.key.hashCode(), newCapacity);
					e.next = newTable[i];
					newTable[i] = e;
					e = next;
				} while (e != null);
	...
	private class HashIterator implements Iterator<T> {
		...
		public void remove() {
			...
			current = null;
			NonCashingHashSet.this.remove(k);
		}
	}
}