...
public final class Vector3 implements Comparable<Vector3>, Serializable {
	private static final WeakHashSet<Vector3> vectors = new WeakHashSet<Vector3>();

	// Constants
	public final static Vector3 Zero = intern(0, 0, 0);
	public final static Vector3 X = intern(1, 0, 0), Y = intern(0, 1, 0), Z = intern(0, 0, 1);
	...
	public Vector3(final float x, final float y, final float z, final float a) {
		this(x * a, y * a, z * a);
	}

	// Static Methods
	/** Constructs vector from String using defaultFormat.<br/>
	 *  Result is interned! */
	...
	public static Vector3 mul(final float x, final float y, final float z, final Matrix3 m) {
		return new Vector3(x * m.a.x + y * m.b.x + z * m.c.x, x * m.a.y + y * m.b.y + z * m.c.y, x * m.a.z + y * m.b.z
				+ z * m.c.z);
	}

	/** m must be diagonal!
	 *  @return transposed(this) * m */
	public Vector3 mulD(final Matrix3 m) {
		return new Vector3(x * m.a.x, y * m.b.y, z * m.c.z);
	}

	/** transposed(this) * transposed(m) */
	public Vector3 mulT(final Matrix3 m) {
	...
	public static Vector3 unitz(final float x, final float y, final float z) {
		final float q = 1 / GMath.sqrt(x * x + y * y + z * z);

		final float xm = x * q;
		if (!GMath.isFinite(xm)) return Vector3.Zero;

		final float ym = y * q;
		if (!GMath.isFinite(ym)) return Vector3.Zero;

		final float zm = z * q;
		if (!GMath.isFinite(zm)) return Vector3.Zero;
	...
	public float angle(final Vector3 a) {
		return (float) Math.acos(dot(a) / GMath.sqrt(square() + a.square()));
	}
	...
	// Non-Math
	/** Returns identical references for equal vectors. */
	...
	public final boolean equals(final Vector3 a, final float e) {
		return Math.abs(x - a.x) <= e && Math.abs(y - a.y) <= e && Math.abs(z - a.z) <= e;
	}
