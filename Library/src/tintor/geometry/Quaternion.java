...
public final class Quaternion implements Serializable {
	...
	public static Quaternion make(final Vector3 a, final Vector3 b) {
		...
		return make(axis, (float) Math.acos(a.dot(b) / GMath.sqrt(a.square() * b.square())));
	}
	...
	public Quaternion unit() {
		return this == Identity ? Identity : unit(w, x, y, z);
	}

	public static Quaternion unit(final float w, final float x, final float y, final float z) {
		final float e = 1 / GMath.sqrt(w * w + x * x + y * y + z * z);

		final float we = w * e;
		if (!GMath.isFinite(we)) return Quaternion.Zero;
		final float xe = x * e;
		if (!GMath.isFinite(xe)) return Quaternion.Zero;
		final float ye = y * e;
		if (!GMath.isFinite(ye)) return Quaternion.Zero;
		final float ze = z * e;
		if (!GMath.isFinite(ze)) return Quaternion.Zero;

		return new Quaternion(we, xe, ye, ze);
	}
	...
	public static Quaternion slerp(final Quaternion p, final Quaternion q, final float t) {
		...
		if (d >= 0) {
			final float a = (float) Math.acos(d), k = 1 / GMath.sin(a);
			return combine(p, GMath.sin(a - t * a) * k, q, GMath.sin(t * a) * k);
		}
		final float a = (float) Math.acos(-d);
	...
	public static Quaternion diagonalize(final Matrix3 m, int steps) {
		...
		while (steps-- > 0) {
			...
			final Matrix3 m1 = q.matrix();
			final Matrix3 d = m1.mul(m).mulT(m1);
	...
	public boolean equals(final Quaternion a) {
		final float q = GMath.square(x - a.x) + GMath.square(y - a.y) + GMath.square(z - a.z) + GMath.square(w - a.w);
		return q <= Side.eps * Side.eps;
	}
