...
public class QuickHull3D {
	 ...
	 * coplanar.
	 */
	public QuickHull3D(Vector3[] points) throws IllegalArgumentException {
	 ...
	 * coplanar.
	 */
	public void build(Vector3[] points) throws IllegalArgumentException {
		build(points, points.length);
	}
	 ...
	 */
	public void build(Vector3[] points, int nump) throws IllegalArgumentException {
		if (nump < 4) {
			throw new IllegalArgumentException("Less than four input points specified");
		}
	...
	protected void setPoints(Vector3[] pnts, int nump) {
		for (int i = 0; i < nump; i++) {
			Vertex vtx = pointBuffer[i];
			vtx.pnt = new Vector3(pnts[i].x, pnts[i].y, pnts[i].z);
			vtx.index = i;
		}
	}
	...
	protected void computeMaxAndMin() {
		...
		for (int i = 1; i < numPoints; i++) {
			...
			if (pnt.x > max.x) {
				max = new Vector3(pnt.x, max.y, max.z);
				maxVtxs[0] = pointBuffer[i];
			} else if (pnt.x < min.x) {
				min = new Vector3(pnt.x, min.y, min.z);
				minVtxs[0] = pointBuffer[i];
			}
			if (pnt.y > max.y) {
				max = new Vector3(max.x, pnt.y, max.z);
				maxVtxs[1] = pointBuffer[i];
			} else if (pnt.y < min.y) {
				min = new Vector3(min.x, pnt.y, min.z);
				minVtxs[1] = pointBuffer[i];
			}
			if (pnt.z > max.z) {
				max = new Vector3(max.x, max.y, pnt.z);
				maxVtxs[2] = pointBuffer[i];
			} else if (pnt.z < min.z) {
				min = new Vector3(min.x, min.y, pnt.z);
				maxVtxs[2] = pointBuffer[i];
			}
		}
		...
		if (explicitTolerance == AUTOMATIC_TOLERANCE) {
			tolerance = 3
					...
					* (Math.max(Math.abs(max.x), Math.abs(min.x))
							+ Math.max(Math.abs(max.y), Math.abs(min.y)) + Math.max(Math.abs(max.z), Math
							.abs(min.z)));
		} else {
	...
	protected void createInitialSimplex() throws IllegalArgumentException {
		...
		double diff = maxVtxs[0].pnt.x - minVtxs[0].pnt.x;
		if (diff > max) {
			max = diff;
			imax = 0;
		}
		...
		diff = maxVtxs[2].pnt.z - minVtxs[2].pnt.z;
		if (diff > max) {
			max = diff;
			imax = 2;
		}
		...
		double maxSqr = 0;
		u01 = vtx[1].pnt.sub(vtx[0].pnt);
		u01 = u01.unit();
		for (int i = 0; i < numPoints; i++) {
			diff02 = pointBuffer[i].pnt.sub(vtx[0].pnt);
			xprod = u01.cross(diff02);
			double lenSqr = xprod.quad();
			if (lenSqr > maxSqr) {
				maxSqr = lenSqr;
				vtx[2] = pointBuffer[i];
				nrml = xprod;
	...
	protected void calculateHorizon(Vector3 eyePnt, HalfEdge edge0, Face face, Vector<HalfEdge> horizon) {
		int cnt = 0;
		//	   oldFaces.add (face);
