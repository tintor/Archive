...
class Face {
	...
	public void computeCentroid(Vector3 centroid) {
		...
		centroid = centroid.div(numVerts);
	}

	public void computeNormal(Vector3 normal, double minArea) {
		...
		if (area < minArea) {
			...
			Vector3 u = new Vector3(p2.x - p1.x, p2.y - p1.y, p2.z - p1.z).div(lenMax);
			normal = normal.sub(u, normal.dot(u)).unit();
		}
	}

	public void computeNormal(Vector3 normal) {
		...
		while (he2 != he0) {
			...
			normal = normal.add(new Vector3(d1y * d2z - d1z * d2y, d1z * d2x - d1x * d2z, d1x * d2y - d1y * d2x));
	...
	void checkConsistency() {
		...
		do {
			...
			if (hedgeOpp.head() != hedge.tail() || hedge.head() != hedgeOpp.tail()) {
				throw new RuntimeException("face " + getVertexString() + ": " + "half edge "
