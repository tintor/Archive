...
class XServo3 {
	private final YServo servoX = new YServo(), servoY = new YServo(),
			servoZ = new YServo();

	Vector3 eval(final Vector3 r, final Vector3 v) {
		return new Vector3(servoX.eval(r.x, v.x), servoY.eval(r.y, v.y), servoZ
				.eval(r.z, v.z));
	}
...
class XServo {
	...
	float eval(final float r, final float v) {
		ir += r;
		iv += v;
		final float a = PR * r + IR * ir + DR * (r - pr) + PV * v + IV * iv
				+ DV * (v - pv);
		pr = r;
		pv = v;
		return a;
