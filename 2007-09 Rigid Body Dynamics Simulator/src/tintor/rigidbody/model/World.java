...
public class World {
	public static final Body Space = new Body(Vector3.Zero,
			Quaternion.Identity, Shape.box(1e5f, 1e5f, 1e5f), 1e10f);
	...
	public void step(final int n) {
		...
		if (showTiming)
			System.out.printf("physics: %s, detector: %s\n", timer,
					detector.timer);
	...
	private final Shape nail = new Shape(ConvexPolyhedrons.pyramid(3, 0.05f,
			0.08f));
	...
	// public void explosion(final Vector3 position, final Vector3 velocity,
	// final float pressure, final float minRadius,
