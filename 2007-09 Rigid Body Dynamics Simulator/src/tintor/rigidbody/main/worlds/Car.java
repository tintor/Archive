...
public class Car extends World {
	...
	void flipFlop() {
		...
		final Body b = new Body(new Vector3(10, sizeY * 0.666, 150), Quaternion.axisY(GMath.deg2rad(90)), ramp, 0.1f);
		b.dfriction = World.Space.dfriction;
		b.setAngVelocity(new Vector3(-1, 0, 0));
		b.sfriction = World.Space.sfriction;
		b.elasticity = 0;
		b.color = GLA.gray;
		add(b);
	...
	void cone(final double x, final double z) {
		final Body b = new Body(new Vector3(x, 10.0 / 3, z), Quaternion.axisY((float)(Math.random() * Math.PI * 2)).mul(
				Quaternion.axisX((float)(-Math.PI / 2))), cone, 1e10f);
		b.dfriction = World.Space.dfriction;
		b.sfriction = World.Space.sfriction;
		b.elasticity = World.Space.elasticity;
		b.state = Body.State.Fixed;
	...
	Body ramp(final float x, final float z, final float a, final float sizeX, final float sizeY, final float sizeZ) {
		...
		final Body b = new Body(new Vector3(x, sizeY / 3, z), Quaternion.axisY(GMath.deg2rad(a)), ramp, 1e10f);
		b.dfriction = World.Space.dfriction;
		b.sfriction = World.Space.sfriction;
		b.elasticity = 0;
		b.color = GLA.brown;
	...
	@Override public void keyDown(final int key) {
		...
		case KeyEvent.VK_RIGHT:
			...
			break;
		case KeyEvent.VK_B:
			final Body b = new Body(new Vector3(100 + Math.random() * 5, 40, Math.random() * 5), Quaternion.axisX((float)Math
					.random()), box, 5);
