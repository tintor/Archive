...
public class LinkedFile {
	...
	public static LinkedFile SetupFromExistingStream(java.io.RandomAccessFile fromfile) throws Exception {
		return SetupFromExistingStream(fromfile, (long) 0);
	}

	public static LinkedFile SetupFromExistingStream(java.io.RandomAccessFile fromfile, long StartSeek) throws Exception {
	...
	public static LinkedFile InitializeLinkedFileInStream(java.io.RandomAccessFile fromfile, int buffersize, long StartSeek)
		...
		result.buffers = BufferFile.InitializeBufferFileInStream(fromfile, buffersize + BUFFEROVERHEAD, StartSeek
				+ result.headersize);
		return result;
	}
	...
	public void Recover(Hashtable ChunksInUse, boolean FixErrors) throws Exception {
	...
	public void checkStructure(Hashtable ChunksInUse, boolean FixErrors) throws Exception {
		...
		for (long buffernumber = 0; buffernumber < LastBufferNumber; buffernumber++) {
			...
			if (thetype == HEAD) {
				...
				while (bodybuffernumber != NULLBUFFERPOINTER) {
					...
					long NextBufferNumber = ((Long) buffernumberToNext.get(new Long(bodybuffernumber)))
							.longValue();
