...
public class Controller extends MouseAdapter implements GLEventListener, MouseMotionListener, KeyListener,
	...
	// GLEventListener
	@Override public synchronized void init(final GLAutoDrawable drawable) {
	...
	@Override public synchronized void display(final GLAutoDrawable drawable) {
		GLA.gl = drawable.getGL();
		view.display();
	}

	@Override public synchronized void displayChanged(final GLAutoDrawable drawable, final boolean arg1, final boolean arg2) {}

	@Override public synchronized void reshape(final GLAutoDrawable drawable, final int x, final int y, final int w,
			final int h) {
		width = w;
		height = h;
