...
public class Billiards extends World {
	...
	void table() {
		final double hole = 3, seg = 20, w = 2;
		...
		part(new Vector3(0, 0, -seg - hole - w), s, Math.PI / 2);
		part(new Vector3(0, 0, seg + hole + w), s, Math.PI / 2);
	...
	void part(final Vector3 pos, final Shape shape, final double angle) {
		final Body b = new Body(pos, Quaternion.axisY(angle), shape, 1e10);
		b.state = Body.State.Fixed;
		b.color = GLA.brown;
	...
	void hit(final double x, final double z) {
		que.linVel = que.linVel.add(new Vector3(x, 0, z));
