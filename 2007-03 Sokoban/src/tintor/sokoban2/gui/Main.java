...
import tintor.sokoban2.Deadlock;
import tintor.sokoban2.Key;
import tintor.sokoban2.Monitor;
import tintor.sokoban2.Solver;
...
public class Main implements PaintListener, KeyListener, MouseMoveListener, MouseTrackListener, MouseListener,
	...
	final Properties properties = Properties.properties;
	final Shell shell = new Shell();
	final Model model = new Model();
	final Canvas canvas = new Canvas(shell, SWT.DOUBLE_BUFFERED);
	final Text console = new Text(shell, SWT.MULTI | SWT.WRAP | SWT.BORDER | SWT.V_SCROLL | SWT.DOUBLE_BUFFERED);

	private Action check(final String name, final boolean init) {
		final int a = name.lastIndexOf('.');
		int d = name.indexOf('|');
		if (d == -1) d = name.length();
		final String text = name.substring(a + 1, d);
		...
		return new Action(text, d != name.length() ? name.substring(d + 1) : null, Action.Check, property);
	}
	...
	void createDirectoryMenu(final MenuItem parent, final File directory) {
	...
	final Action Reset = new Action("Reset", "Multiply") {
		...
		@Override public void onSelect() {
			final Thread a = new Thread() {
				@Override public void run() {
					...
					Monitor.start();
					final Key result = Solver.ida(model.key);
					Monitor.stop();
	...
	private final XMenu View = new XMenu("View", "Problem Set", null, check("show.Agent|Alt+A", true), check(
			"show.Boxes|Alt+B", true), check("show.IDs|Alt+I", false), check("show.Dead Cells|Alt+D", true), check(
			"show.Goals|Alt+G", true), check("show.Matching Goals|Alt+M", false), check("show.Push Distances|Alt+P",
			false), check("show.Articulations|Alt+R", false), check("show.Unreachable|Alt+U", true), check(
			"show.Tunnel Lengths|Alt+T", false));

	private final XMenu Settings = new XMenu("Settings", check("Remove Deadends", null), check("Compress Tunnels", null),
			null, check("Separate Simple Goal Rooms", null), null, new XMenu("Heuristic", "? Closest Goals",
					"? Matching Goals"), new XMenu("Deadlock testing", check("Fast Block", Deadlock.fastBlock),
					check("Frozen Boxes", Deadlock.frozenBoxes)), check("Macro Expansion", null));
	...
	public Main() {
		try {
			...
			final OutputStream consoleOutputStream = new BufferedOutputStream(new DoubleOutputStream(
					new WriterOutputStream(new TextWidgetWriter(console)), new FileOutputStream(consoleFile)));
		...
		// init properties
		//properties.start(Properties.DefaultFile, false, false);
		...
		// init console
		console.setFont(new Font(null, properties.instance(Main.class, "console.font.name", "Courier New"), properties.instance(
				Main.class, "console.font.size", 9), SWT.NORMAL));
		final Color color = console.getBackground();
		console.setEditable(false);
		console.setBackground(color);

		// init shell
		shell.setMaximized(properties.instance(Main.class, "shell.maximized", true));

		final List<Integer> location = properties.instance(Main.class, "shell.location", Arrays.asList(0, 0));
	...
	void load(final String file, final int levelId) {
	...
	@Override public void paintControl(final PaintEvent e) {
		...
		for (final Cell c : model.cachedCells) {
			...
			if (properties.getBool("show.boxes") && model.key.hasBox(c))
			...
			if (c.distance(Dir.East) > 1) {
				e.gc.setBackground(c.dead() || c.east().dead() ? gray60 : gray75);
				e.gc.fillRectangle(cellSize, tunnelOff, (c.distance(Dir.East) - 1) * cellSize, tunnelSize);
			}
			if (c.distance(Dir.South) > 1) {
				e.gc.setBackground(c.dead() || c.south().dead() ? gray60 : gray75);
				e.gc.fillRectangle(tunnelOff, cellSize, tunnelSize, (c.distance(Dir.South) - 1) * cellSize);
			}
			...
			// Articulations
			if (properties.getBool("show.articulations") && c.articulation()) {
			...
			if (properties.getBool("show.goals") && c.goal) {
			...
			if (properties.getBool("show.unreachable") && !model.cachedReachable.contains(c) && !canReachAndPush(c)) {
			...
			if (c.flag != 0) {
				...
				final double k = 0.4;
				final int off = (int) (cellSize * (1 - k) / 2), rad = (int) (cellSize * k);
				e.gc.drawRectangle(off, off, rad, rad);
			}

			if (properties.getBool("show.ids")) {
				e.gc.setForeground(white);
				SWT2D.drawText(e.gc, cellSize / 2, cellSize / 2, Align.Center, Align.Center, c.id());
			}
		}
		...
		if (cursor != null) {
			...
			if (properties.getBool("show.tunnel_lengths")) {
				...
				for (final Dir dir : Dir.values())
					SWT2D.drawText(e.gc, (1 + dir.dx) * cellSize / 2, (1 + dir.dy) * cellSize / 2, Align.Center,
							Align.Center, cursor.distance(dir));
